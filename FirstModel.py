...
for j in range(0,len(results)):
	for i in range(0,len(results[j])):
		try:
			tpp = RunoffElection.GetTPP(ComputeSeatSwings.ApplySwings(results[j][i], 2010, swing_dict, basic_pref_flows))
			...
			if (tpp > 50.00) and (actual_tpp < 50.00):
				...
				highlight = ''
			print results[j][i].name, actual_results[j]['seat'][i], tpp, actual_tpp, highlight
